		...
		?>>
			...
			?><div class="sc_price_item_info"><?php
				...
				if (!empty($item['description'])) {
					...
					?><div class="sc_price_item_description"><?php
						foreach ($item['description'] as $str) {
							?><span><?php echo wp_kses(trx_addons_parse_codes($str), 'lighthouseschool_kses_content'); ?></span><?php
						}
					?></div><?php
				}
				if (!empty($item['price'])) {
					...
					?><div class="sc_price_item_price"><?php
						if (!empty($item['before_price'])) {
							?><span class="sc_price_item_price_before"><?php echo wp_kses(trx_addons_parse_codes($item['before_price']), 'lighthouseschool_kses_content'); ?></span><?php
						}
						?><span class="sc_price_item_price_value"><?php echo wp_kses($parts[0], 'lighthouseschool_kses_content'); ?></span><?php
						if (count($parts) > 1 && $parts[1]!='') {
							?><span class="sc_price_item_price_decimals"><?php echo wp_kses($parts[1], 'lighthouseschool_kses_content'); ?></span><?php
						}
						if (!empty($item['after_price'])) {
							?><span class="sc_price_item_price_after"><?php echo wp_kses(trx_addons_parse_codes($item['after_price']), 'lighthouseschool_kses_content'); ?></span><?php
						}
					?></div><?php
				}
				...
				if (!empty($item['link']) && !empty($item['link_text'])) {
					?><a href="<?php echo esc_url($item['link']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_price_item_link_classes', 'sc_price_item_link sc_button')); ?>" target="_blank"><?php
