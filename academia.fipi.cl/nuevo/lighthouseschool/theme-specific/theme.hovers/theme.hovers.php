...
if (!function_exists('lighthouseschool_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'lighthouseschool_hovers_theme_setup3', 3 );
	function lighthouseschool_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		lighthouseschool_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'lighthouseschool'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'lighthouseschool') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'lighthouseschool'),
					'slide_left'	=> esc_html__('Slide from Left',	'lighthouseschool'),
					'slide_right'	=> esc_html__('Slide from Right',	'lighthouseschool'),
					'slide_top'		=> esc_html__('Slide from Top',		'lighthouseschool'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'lighthouseschool'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'lighthouseschool'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'lighthouseschool') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'lighthouseschool'),
					'icon'	=> esc_html__('Icon',	'lighthouseschool'),
					'icons'	=> esc_html__('Icons',	'lighthouseschool'),
					'zoom'	=> esc_html__('Zoom',	'lighthouseschool'),
					'fade'	=> esc_html__('Fade',	'lighthouseschool'),
					'slide'	=> esc_html__('Slide',	'lighthouseschool'),
					'pull'	=> esc_html__('Pull',	'lighthouseschool'),
					'border'=> esc_html__('Border',	'lighthouseschool')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('lighthouseschool_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'lighthouseschool_hovers_theme_setup9', 9 );
	function lighthouseschool_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'lighthouseschool_hovers_frontend_scripts', 1010 );
		add_filter( 'lighthouseschool_filter_localize_script','lighthouseschool_hovers_localize_script' );
		add_filter( 'lighthouseschool_filter_merge_scripts',	'lighthouseschool_hovers_merge_scripts' );
		add_filter( 'lighthouseschool_filter_merge_styles',	'lighthouseschool_hovers_merge_styles' );
		add_filter( 'lighthouseschool_filter_get_css', 		'lighthouseschool_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'lighthouseschool_hovers_frontend_scripts' ) ) {
	
	function lighthouseschool_hovers_frontend_scripts() {
		if ( lighthouseschool_is_on(lighthouseschool_get_theme_option('debug_mode')) && lighthouseschool_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'lighthouseschool-hovers', lighthouseschool_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( lighthouseschool_is_on(lighthouseschool_get_theme_option('debug_mode')) && lighthouseschool_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'lighthouseschool-hovers',  lighthouseschool_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('lighthouseschool_hovers_merge_scripts')) {
	
	function lighthouseschool_hovers_merge_scripts($list) {
...
if (!function_exists('lighthouseschool_hovers_merge_styles')) {
	
	function lighthouseschool_hovers_merge_styles($list) {
...
if (!function_exists('lighthouseschool_hovers_localize_script')) {
	
	function lighthouseschool_hovers_localize_script($arr) {
		$arr['button_hover'] = lighthouseschool_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('lighthouseschool_hovers_add_icons') ) {
	function lighthouseschool_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'lighthouseschool') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'lighthouseschool');
						else
							esc_html_e('Details', 'lighthouseschool');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-img_preview"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'lighthouseschool_hovers_get_css' ) ) {
	
	function lighthouseschool_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
.sc_button_hover_style_dark.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_dark']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_hover']} !important; }
.sc_button_hover_style_dark.sc_button_hover_slide_right {		background: linear-gradient(to left,	{$colors['text_dark']} 50%, {$colors['text_hover']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_hover']} !important; }
.sc_button_hover_style_dark.sc_button_hover_slide_top {			background: linear-gradient(to bottom,	{$colors['text_dark']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_hover']} !important; }
.sc_button_hover_style_dark.sc_button_hover_slide_bottom {		background: linear-gradient(to top,		{$colors['text_dark']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_hover']} !important; }

.sc_button_hover_style_inverse.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['inverse_dark']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_hover']} !important; }
.sc_button_hover_style_inverse.sc_button_hover_slide_right {	background: linear-gradient(to left,	{$colors['inverse_dark']} 50%, {$colors['text_hover']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_hover']} !important; }
.sc_button_hover_style_inverse.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['inverse_dark']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_hover']} !important; }
.sc_button_hover_style_inverse.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['inverse_dark']} 50%, {$colors['text_hover']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_hover']} !important; }
