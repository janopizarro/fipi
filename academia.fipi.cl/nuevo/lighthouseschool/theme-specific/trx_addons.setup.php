...
------------------------------------------------------------------------------- */
if (!function_exists('lighthouseschool_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'lighthouseschool_trx_addons_theme_specific_components');
	function lighthouseschool_trx_addons_theme_specific_components($enable=false) {
		return LIGHTHOUSESCHOOL_THEME_FREE
					? false		// Free version
...
if (!function_exists('lighthouseschool_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'lighthouseschool_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'lighthouseschool_trx_addons_theme_specific_setup1', 1 );
	function lighthouseschool_trx_addons_theme_specific_setup1() {
		if (lighthouseschool_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'lighthouseschool_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'lighthouseschool_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'lighthouseschool_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'lighthouseschool_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'lighthouseschool_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'lighthouseschool_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'lighthouseschool_trx_addons_cv_enable' ) ) {
	
	function lighthouseschool_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'lighthouseschool_trx_addons_demo_enable' ) ) {
	
	function lighthouseschool_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'lighthouseschool_trx_addons_edd_themes_market_enable' ) ) {
	
	function lighthouseschool_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'lighthouseschool_trx_addons_api_list' ) ) {
	
	function lighthouseschool_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'lighthouseschool_trx_addons_cpt_list' ) ) {
	
	function lighthouseschool_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'lighthouseschool_trx_addons_sc_list' ) ) {
	
	function lighthouseschool_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'lighthouseschool_trx_addons_widgets_list' ) ) {
	
	function lighthouseschool_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'lighthouseschool_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'lighthouseschool_trx_addons_menu_cache');
	function lighthouseschool_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('lighthouseschool_trx_addons_localize_script')) {
	...
	function lighthouseschool_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = lighthouseschool_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'lighthouseschool_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'lighthouseschool_trx_addons_sc_type', 10, 2);
	function lighthouseschool_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
        if ($sc == 'trx_sc_services') {
            $list['numbered'] = esc_html__('Numbered', 'lighthouseschool');
            $list['blinked'] = esc_html__('Blinked', 'lighthouseschool');
        }
        if ($sc == 'trx_sc_events') {
            $list['full'] = esc_html__('Full', 'lighthouseschool');
        }
        if ($sc == 'trx_sc_content') {
            $list['shadowed'] = esc_html__('Shadowed', 'lighthouseschool');
        }
        if ($sc == 'trx_sc_testimonials') {
            $list['green'] = esc_html__('Green', 'lighthouseschool');
        }
		return $list;
	}
}
...
if ( !function_exists( 'lighthouseschool_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'lighthouseschool_trx_addons_sc_atts', 10, 2);
	function lighthouseschool_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'lighthouseschool_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'lighthouseschool_trx_addons_sc_map', 10, 2);
	function lighthouseschool_trx_addons_sc_map($params, $sc) {
		...
		if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
			...
			$params['params'][] = array(
					'param_name' => 'scheme',
					'heading' => esc_html__('Color scheme', 'lighthouseschool'),
					'description' => wp_kses_data( __('Select color scheme to decorate this block', 'lighthouseschool') ),
					'group' => esc_html__('Colors', 'lighthouseschool'),
					'admin_label' => true,
					'value' => array_flip(lighthouseschool_get_list_schemes(true)),
					'type' => 'dropdown'
				);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading' => esc_html__('Color style', 'lighthouseschool'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'lighthouseschool') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(lighthouseschool_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'lighthouseschool_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'lighthouseschool_trx_addons_sow_map', 10, 2);
	function lighthouseschool_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'lighthouseschool'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'lighthouseschool') ),
				'options' => lighthouseschool_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			lighthouseschool_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			lighthouseschool_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'lighthouseschool_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'lighthouseschool_trx_addons_sc_output', 10, 4);
	function lighthouseschool_trx_addons_sc_output($output, $sc, $atts, $content) {
		
		if (in_array($sc, array('trx_sc_action'))) {
			if (!empty($atts['scheme']) && !lighthouseschool_is_inherit($atts['scheme']))
				$output = str_replace('class="sc_action ', 'class="sc_action scheme_'.esc_attr($atts['scheme']).' ', $output);
			if (!empty($atts['color_style']) && !lighthouseschool_is_inherit($atts['color_style']))
...
if ( !function_exists( 'lighthouseschool_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'lighthouseschool_trx_addons_sc_item_title_tag');
	function lighthouseschool_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'lighthouseschool_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'lighthouseschool_trx_addons_sc_item_button_args', 10, 3);
	function lighthouseschool_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'lighthouseschool_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'lighthouseschool_trx_addons_slider_title', 10, 2 );
	function lighthouseschool_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'lighthouseschool_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'lighthouseschool_trx_addons_sc_googlemap_styles');
	function lighthouseschool_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'lighthouseschool');
		return $list;
	}
}
...
if ( !function_exists( 'lighthouseschool_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'lighthouseschool_trx_addons_tiny_mce_style_formats');
	function lighthouseschool_trx_addons_tiny_mce_style_formats($style_formats) {
		// Add style 'Arrow' to the 'List styles'
		// Remove 'false &&' from the condition below to add new style to the list
		if (true && is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
                if ( $v['title'] == esc_html__('Inline', 'lighthouseschool') ) {
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Contacts title', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_contacts_title'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Contacts phone number', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_contacts_phone'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Promo large', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_promo_large'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Promo small', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_promo_small'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Promo italic', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_promo_italic'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Events italic', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_events_italic'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Events bold', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_events_bold'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Events text', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_events_text'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Events small', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_events_small'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('TimeLine year', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_timeline_year'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('TimeLine title', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_timeline_title'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Shop promo large', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_shop_promo_large'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Shop promo small', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_shop_promo_small'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Shop promo italic', 'lighthouseschool'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_shop_promo_italic'
                    );
                }
			}
		}
...
if ( !function_exists( 'lighthouseschool_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'lighthouseschool_filter_allow_override_header_image', 'lighthouseschool_trx_addons_allow_override_header_image' );
	function lighthouseschool_trx_addons_allow_override_header_image($allow) {
		return lighthouseschool_is_team_page() ? false : $allow;
	}
}

// Hide sidebar on the team and portfolio pages
if ( !function_exists( 'lighthouseschool_trx_addons_sidebar_present' ) ) {
	add_filter( 'lighthouseschool_filter_sidebar_present', 'lighthouseschool_trx_addons_sidebar_present' );
	function lighthouseschool_trx_addons_sidebar_present($present) {
		return !is_single() && lighthouseschool_is_team_page() ? false : $present;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'lighthouseschool_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'lighthouseschool_trx_addons_thumb_size', 10, 2);
	function lighthouseschool_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = lighthouseschool_get_thumb_size('big');
		return $thumb_size;
	}
}
...
if (!function_exists('lighthouseschool_trx_addons_override_options_fields')) {
	add_filter( 'trx_addons_filter_override_options_fields', 'lighthouseschool_trx_addons_override_options_fields', 10, 2);
	function lighthouseschool_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'lighthouseschool'),
				"desc" => wp_kses_data( __("Team member's email", 'lighthouseschool') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
