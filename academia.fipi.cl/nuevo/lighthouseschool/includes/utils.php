...
if ( ! function_exists( 'lighthouseschool_array_delete_by_value' ) ) {
	function lighthouseschool_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'lighthouseschool_get_value_gpc' ) ) {
	function lighthouseschool_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'lighthouseschool_get_value_gps' ) ) {
	function lighthouseschool_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'lighthouseschool_add_protocol' ) ) {
	function lighthouseschool_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? lighthouseschool_get_protocol() . ':' . $url : $url;
	}
}

// Remove protocol part from URL
if (!function_exists('lighthouseschool_remove_protocol_from_url')) {
	function lighthouseschool_remove_protocol_from_url( $url, $complete = true ) {
