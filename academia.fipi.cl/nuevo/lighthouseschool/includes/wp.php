...
if ( !function_exists( 'lighthouseschool_get_nav_menu' ) ) {
	function lighthouseschool_get_nav_menu($location='', $menu = '', $depth=0, $custom_walker=false) {
		static $list = array();
...
if ( ! function_exists( 'lighthouseschool_get_post_terms' ) ) {
	function lighthouseschool_get_post_terms( $delimiter = ', ', $id = false, $taxonomy = 'category', $links = true ) {
		$output = '';
		if ( empty( $id ) ) {
			$id = get_the_ID();
		}
		if ( empty( $taxonomy ) ) {
			$taxonomy = lighthouseschool_get_post_type_taxonomy( get_post_type( $id ) );
		}
				...
				$output .= ( ! empty( $output )
							...
							. ( $links
								? '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '"'
										...
										. ' title="' . sprintf( esc_attr__( 'View all posts in %s', 'lighthouseschool' ), $term->name ) . '"'
										. '>'
								: '<span>'
								)
								. apply_filters( 'lighthouseschool_filter_term_name', $term->name, $term )
							. ( $links
								? '</a>'
								: '</span>'
								);
...
/* wp_kses handlers
----------------------------------------------------------------------------------------------------- */
if ( ! function_exists( 'lighthouseschool_kses_allowed_html' ) ) {
	add_filter( 'wp_kses_allowed_html', 'lighthouseschool_kses_allowed_html', 10, 2);
	function lighthouseschool_kses_allowed_html($tags, $context) {
	  if ( in_array( $context, array( 'lighthouseschool_kses_content', 'trx_addons_kses_content' ) ) ) {
		 $tags = array(
...
if ( ! function_exists('lighthouseschool_get_privacy_text' ) ) {
	function lighthouseschool_get_privacy_text() {
		...
		return apply_filters( 'lighthouseschool_filter_privacy_text', wp_kses(
				$privacy_text
