...
$preguntas = get_post_meta($id_curso, 'preguntas_unidad_'.$unidad.'', true);
foreach ($preguntas as $key => $pregunta) { 
    $altCorrectas[] = $pregunta['alternativa_correcta_unidad_'.$unidad];
}
...
for ($i = 0; $i <= $cant; $i++) {
    ...
    if(isset($altCorrectas[$i])){
        ...
        $consulta = $wpdb->get_results(" SELECT * FROM $tableName WHERE `id_user` = $id_user AND `id_curso` = $id_curso AND `n_preg` = $x AND `unidad` = $unidad LIMIT 0,1 ");
