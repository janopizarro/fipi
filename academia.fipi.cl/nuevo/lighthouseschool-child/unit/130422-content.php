...
if(count(getUnitsCourse($id_course)) > 0){
    ...
    foreach(getUnitsCourse($id_course) as $res){ $u++;
        ...
        if($docs && count($docs) > 0){
            ...
            $html .= '<strong style="padding-left: 30px;">Lectura obligatoria:</strong><small style="display: block; font-size: 14px; padding-left: 30px;">'.get_post_meta($ID, 'adicional_lectura_obligatoria', true).'</small><nav style="display: flex; padding-left: 18px;">';
        ...
        // PREGUNTAS
        $html .= '<ul class="pregunta"><strong style="padding-left: 30px; color: #bd2bbd;">Evaluación formativa unidad 0'.$u.'</strong>';
        ...
        foreach ($question_unit as $key => $question) { $x++;
            ...
            $html .= '<li class="invoices--flex question_unidad_'.$u.'_'.$x.' '.$classQuestion.'">'.$question_.' <b class="helper_grupo helper_grupo--info helper_grupo_unidad_'.$u.'_'.$x.' '.$helperClass.'" style="display: none;">'.$message.'</b></li>';
            ...
            foreach($alternatives_ as $alternative){ $i++;
                ...
                $html .= '<li>';
                    ...
                    $html .= '<label for="alernativa_'.$i.'_preg_'.$x.'_unidad_'.$u.'" style="'.$checkboxStatus.' '.$checked_style_.'"><span style="background: #bc73bd; min-width: 10px; min-height: 10px; display: inline-block; text-align: center; width: 20px; border-radius: 10px; color: #FFFFFF;">'.letras()[$i]."</span> ".$alternative.'</label>';
        ...
        if(!haveFinishedCourse( $id_user, $id_curso )){
            ...
            $html .= '<button class="button finalizar_unidad finalizar_unidad_'.$u.'" type="button" data-nextunit="'.getNext($ID, $id_course).'" data-unit="'.$ID.'" data-id="'.$u.'" data-cant="'.count($question_unit).'" style="font-size: 15px; margin: 25px;">Finalizar</button>';
    
        }
    
        $html .= '<p id="load_unit_0'.$u.'" style="display:none; font-size: 13px; margin: 10px; color: #3ca4ce; font-family: inherit;"><img src="'.get_template_directory_uri().'/images/loading.svg" style="width: 20px; margin-right: 7px;"/> Actualizando estado...</p>';
        ...
        $html .= '</div>
            ...
            <div class="card" id="f_course">
                <p id="load_finish" style="display:none; font-size: 13px; margin: 10px; color: #3ca4ce; font-family: inherit;"><img src="'.get_template_directory_uri().'/images/loading.svg" style="width: 20px; margin-right: 7px;"/> Actualizando estado...</p>
            </div>';
    ...
    if(haveFinishedCourse( $id_user, $id_curso )){ ?>
        ...
        <div class="card">
            <div class="card-header" id="h_unidad_dudas">
                <h5 class="mb-0">
                    <button class="btn btn-link unidad_dudas" data-toggle="collapse" data-target="#unidad_dudas" aria-expanded="false" aria-controls="unidad_dudas">
                        Dudas / Comentario
                    </button>
                </h5>
            </div>
            <div id="unidad_dudas" class="collapse" aria-labelledby="h_unidad_dudas" data-parent="#accordion">
                <div class="card-body">
                    <p>Hola <?php echo getDataSession("nombre"); ?>, si quedaste con alguna duda especifica del curso y necesitas apoyo, contáctate con el equipo de la academia Fipi.</p>
                            ...
                            <div class="dashboard-list-box-static" style="box-shadow: none; padding: 0px;">
                                ...
                                <form action="" id="form_duda">
                                    ...
                                    <div class="my-profile">
                                        <input type="hidden" name="nombre" id="nombre" value="<?php echo getDataSession("nombre"); ?>">
                                        <input type="hidden" name="email" id="email" value="<?php echo getDataSession("email"); ?>">
                                        <input type="hidden" name="telefono" id="telefono" value="<?php echo get_user_meta(getDataSession("id"), 'user_telefono' , true ); ?>">
    ...
    if(haveFinishedCourse( $id_user, $id_curso)){
        ...
        <div class="card">
            <div class="card-header" id="h_unidad_evaluacion">
                <h5 class="mb-0">
                    <button class="btn btn-link unidad_evaluacion" data-toggle="collapse" data-target="#unidad_evaluacion" aria-expanded="false" aria-controls="unidad_evaluacion">
            ...
            <div id="unidad_evaluacion" class="collapse" aria-labelledby="h_unidad_evaluacion" data-parent="#accordion">
                <div class="card-body">
                    <p>En una escala del 1 al 5, donde 1 es muy malo y 5 excelente, que puntaje le darías al curso realizado?</p>
                            ...
                            <div class="dashboard-list-box-static" style="box-shadow: none; padding: 0px;">
                                ...
                                <form action="" id="form_evaluacion">
                                    ...
                                    <div class="my-profile">
                                        ...
                                        <input type="hidden" name="id_user" id="id_user" value="<?php echo getDataSession("id"); ?>">
                                        <input type="hidden" name="email" id="email" value="<?php echo getDataSession("email"); ?>">
                                        <input type="hidden" name="curso" id="curso" value="<?php echo the_title(); ?>">
                                        ...
                                        <button class="button margin-top-15" type="button" id="enviar_evaluacion">Enviar evaluación</button>

                                        <!--<small style="display: block; line-height: 19px; font-size: 14px; background: #ffffcf; padding: 10px; margin-top: 12px; border: 1px solid #ffe214; color: #121212;">Enviaremos una encuesta de satisfacción del curso a tu correo, por favor responde para comenzar el proceso de emisión de certificado.</small>-->
                    ...
                    <style>
                    .status_message_ev p{padding: 7px;font-size: 13px;border-radius: 5px;}
                    .warning{background: #d4c516;color: #6f4800;}
                    .error{background: #d41616;color: #ffd0d0;}
                    .info{background: #45c8e6;color: #0f5469;}
                    .success{background: #a1bf42;color: #2d4a00;}
                    ...
                    document.getElementById("enviar_evaluacion").addEventListener("click", function(){
                        ...
                        .then(function(response) {
                            if(response.ok) {
                                ...
                                document.querySelector(".status_message_ev").innerHTML = "<p class='error'>Problema en la llamada ajax!</p>";
                            }
                        })
                        .then(function(res) {
                            ...
                            document.querySelector(".status_message_ev").innerHTML = "<p class='"+class_message+"'>"+message+"</p>";
    ...
    <script src="'.get_template_directory_uri().'/js/sweetalert2.all.min.js"></script>
    <script src="https://player.vimeo.com/api/player.js"></script>
    <script src="'.get_template_directory_uri().'/unit/content.js"></script>
